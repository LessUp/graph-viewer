...
export function VersionHistoryPanel(props: VersionHistoryPanelProps) {
  const { versions, isLoading, onRestore, onDelete, onRename, onCreateSnapshot, onClearAll } =
    props;
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1 rounded-lg bg-sky-50 px-2.5 py-1.5 text-xs font-medium text-sky-600 transition hover:bg-sky-100"
          >
          ...
          {versions.length > 0 && (
            <button
              onClick={onClearAll}
              className="rounded-lg px-2 py-1.5 text-xs text-slate-500 transition hover:bg-slate-100 hover:text-rose-600"
            >
              清空
            </button>
          )}
      ...
      <div className="flex-1 overflow-y-auto">
        {versions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-slate-400">
            ...
            <p className="mt-1 text-xs">系统会自动保存您的修改</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    {editingId === version.id ? (
                      ...
                      <div className="mb-1 flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="hidden text-slate-400 hover:text-slate-600 group-hover:block"
                        >
                    ...
                    {expandedId === version.id && (
                      <div className="mt-2 max-h-40 overflow-y-auto whitespace-pre-wrap rounded bg-slate-100 p-2 font-mono text-[11px] text-slate-600">
                        {version.code}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1 opacity-0 transition group-hover:opacity-100">
                    ...
                    >
                      <ChevronDown
                        className={`h-4 w-4 transition ${expandedId === version.id ? 'rotate-180' : ''}`}
                      />
                    </button>
                    <button
      ...
      {versions.length > 0 && (
        <div className="border-t border-slate-200 px-4 py-2 text-[11px] text-slate-400">
          共 {versions.length} 个版本 ·{versions.filter((v) => !v.autoSave).length} 个手动保存
        </div>
      )}
    </div>
  );
